...
### 11. 游戏策划 Agent (`game-designer.md`)

**职责**:游戏策划专家,专注于微信小游戏设计与开发

**核心功能**:
- ✅ **微信小游戏平台特性**:社交传播、轻量化、技术限制、适合的游戏类型
- ✅ **游戏系统设计**:核心玩法、关卡设计、社交系统、成长系统
- ✅ **留存与变现策略**:次日/7日留存机制、广告变现、内购设计、ARPPU提升
- ✅ **数值平衡**:关卡难度曲线、经济系统平衡、属性成长设计
- ✅ **用户体验优化**:新手引导、反馈系统、性能优化
- ✅ **数据分析指标**:DAU/MAU、ARPU/ARPPU、留存率、付费率
...
### 12. 设计规范审查 Agent (`design-spec-auditor.md`)

**职责**:设计规范审查专家,分析Figma设计稿并对比项目现有设计规范

**核心功能**:
- ✅ **项目设计规范提取**:
  - 从Less变量文件自动提取设计规范（颜色、字体、间距、圆角等）
  - 支持变量引用解析（@link-color: @primary-color）
  - 识别Less函数（lighten、darken、fade等）
- ✅ **Figma设计稿分析**:
  - 调用figma-to-code获取Design Token
  - 支持Figma MCP、API、JSON多种数据源
- ✅ **智能差异对比**:
  - 颜色对比（RGB欧氏距离算法,相似度计算）
  - 字体对比（允许±2px误差）
  - 间距对比（8px网格系统检查）
  - 组件语义对比（主次按钮颜色检查）
- ✅ **对比报告生成**:
  - 按维度分类差异（✅完全匹配 / ⚠️接近 / ❌不符合）
  - 提供修正建议和映射关系
  - 输出修正后的Design Token（使用Less变量）
- ✅ **符合规范的代码生成**:
  - 调用component-generator生成组件
  - 自动替换为项目Less变量
  - 确保代码符合项目设计规范

**工作流程**（5阶段）:
1. 提取项目设计规范（Less变量解析）
2. 获取Figma设计稿数据（调用figma-to-code）
3. 对比差异分析（颜色/字体/间距/组件）
4. 生成对比报告（Markdown表格）
5. 生成符合规范的代码（调用component-generator）
...
除了这 12 个核心 Agents，我们还提供了 **专项技能 Skills**，用于处理特定技术栈和垂直领域的任务。
